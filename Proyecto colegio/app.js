...
matriculadoId += 1;
matriculados.push(new Matriculado(matriculadoId, "Juan", "Perez", 2015998512));
...
matriculadoId += 1;
matriculados.push(
  new Matriculado(matriculadoId, "Marcos Ireneo", "Barbas", 20380831652)
);
...
function Factura(id, numeroFactura, descripcion, monto, idMatriculado, fecha) {
  ...
  this.fecha = fecha;
  this.retencion = 0; // 0 < Factura Liquidada
...
const cargarRetenciones = () => {
  ...
  try {
    ...
    for (let i = 0; i < retenciones.length; i++) {
      ...
      console.log(fechaRetencion.getMonth(),fechaHoy.getMonth(),
  ...
  if (restrinctedPercentage) {
    for (let i = 0; i < facturas.length; i++) {
      ...
      if (
        ...
        /*facturasFacturadas.push(
          new FacturaFacturada(liquidaciones.length + 1, facturas[i].id)
        );*/
  ...
  for (let j = 0; j < matriculados.length; j++) {
    for (let i = 0; i < liquidaciones.length; i++) {
      if (
        ...
        /*
        console.log(liquidaciones[i].id);
        console.log(facturasFacturadas);
        console.log(facturas);
        console.log(liquidaciones);
*/
        data.push("Facturas Incluidas", "Monto correspondiente a la factura");
